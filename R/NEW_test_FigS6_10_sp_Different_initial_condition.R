...
new_proportion_for_distance_from_V1 = 1.5
new_pairs_vertices$arc_distance[1:(total_number_species-1)] <- new_proportion_for_distance_from_V1 * pairs_vertices$arc_distance[1:(total_number_species-1)] 
...
for (i in 1:nrow(A_asym_new2)) {
  A_asym_new2[,i] <- -search_new_vertices_prop_triangle$x[(1+(i-1)*total_number_species):(i*total_number_species)]
}


A_asym_new2

new_vertices_coordinates2 <- vertices_coordinates

for (i in 1:nrow(new_vertices_coordinates2)) {
  new_vertices_coordinates2[i,] <- search_new_vertices_prop_triangle$x[(1+(i-1)*total_number_species):(i*total_number_species)]
}


new_pairs_vertices_aux2 <- t(combn(x=1:total_number_species, m=2)) %>% as_tibble()
new_pairs_vertices_aux2$arc_distance <- NA
for (pair.i in 1:nrow(new_pairs_vertices_aux2)) {
  new_pairs_vertices_aux2$arc_distance[pair.i] <- arc_distance_vertices(new_vertices_coordinates2,
                                                                       new_pairs_vertices_aux2$V1[pair.i],
                                                                       new_pairs_vertices_aux2$V2[pair.i])
}

new_pairs_vertices_aux2$arc_distance - new_pairs_vertices$arc_distance

# Compute new asymmetry index

set.seed(1234)

number_Omega_replicates <- 5000
number_boot_replicates <- number_Omega_replicates

new_isotropy_metrics_results <- isotropy_metrics_4_int_matrix(A_asym_new2, number_Omega_replicates, number_boot_replicates)
new_isotropy_metrics_results

J_index_new <- new_isotropy_metrics_results$isotropy_index_mean
cos_variance_new <- var(cos(new_pairs_vertices_aux2$arc_distance))


# Estimate a new system were arc distance among nodes are proportional to the previous ones
proportions_4_tests <- seq(from = 1.1, to = .1, length.out=9)
...
pairs_vertices2 <- new_pairs_vertices_aux2

for(proportion in proportions_4_tests){
  
  new_pairs_vertices <- pairs_vertices2
  new_pairs_vertices$arc_distance <- proportion * pairs_vertices2$arc_distance
