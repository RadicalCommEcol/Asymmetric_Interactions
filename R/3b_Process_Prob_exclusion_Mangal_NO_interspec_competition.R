...
kingdom_data_aux1 <- read_csv("Data/mangal_processed_data/NO_NAs_mangal_links_processed_kingdom.csv") %>%
...
kingdom_data_aux2 <- read_csv("Data/mangal_processed_data/NO_NAs_mangal_links_processed_kingdom.csv") %>%
...
mangal_plot_network_raw <- read_csv(paste0("Data/mangal_processed_data/NO_NAs_NO_interspec_competition_mangal_boot_networks_rep_",
...
diff_isoprob_excl_data <- mangal_plot_network_final %>% filter(metric == "diff_isoprob_excl")%>% 
  ...
  mutate(species = paste0(network_id,species), isoprob = 1/ number_plant_sp,
         mutualistic_strength = 0.5*gamma_0_max/(degree^0.25),
         total_mutualistic_strength = degree*0.5*gamma_0_max/(degree^0.25))
