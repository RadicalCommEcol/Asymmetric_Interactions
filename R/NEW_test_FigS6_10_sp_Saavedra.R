...
for(proportion in proportions_4_tests){
  ...
  omega_species_A_asym <- NULL
  omega_species_A_asym_lowerlimit <- NULL
  omega_species_A_asym_upperlimit <- NULL
  
  for (species.i in 1:ncol(A_asym_new)) {
    
    A_int_aux_i <-  A_asym_new[-species.i,-species.i] 
    omega_sampled <- anisoFun::Omega_bootstrap(A_int_aux_i, number_Omega_replicates)
    
    omega_species_A_asym <- c(omega_species_A_asym, mean(omega_sampled))
    
    b1<-boot(omega_species_A_asym,function(u,i) mean(u[i]),R=1000)
    ci_omega <- boot.ci(b1,type=c("norm"))
    
    omega_species_A_asym_lowerlimit <- c(omega_species_A_asym_lowerlimit, ci_omega[["normal"]][2])
    omega_species_A_asym_upperlimit <- c(omega_species_A_asym_upperlimit, ci_omega[["normal"]][2])
    
    
  }
  
  # correcction_factor_omega <- 2^(ncol(A_asym_new)-1) # We add a minus one because omega projections contain n-1 species
  # 
  # saavedra_index_new <- var(correcction_factor_omega*omega_species_A_asym)
  
  saavedra_index_new <- var(omega_species_A_asym)
  saavedra_index_new_lowerlimit <- var(omega_species_A_asym_lowerlimit)
  saavedra_index_new_upperlimit <- var(omega_species_A_asym_upperlimit)
